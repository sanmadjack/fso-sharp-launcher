...
public partial class MainWindow: Gtk.Window {
	...
	private registryHandler fs2_registry;
	...
	// Original code after this point, all needs to be reviewed
	public MainWindow () : base (Gtk.WindowType.Toplevel) {
		Build ();
		string system_os = System.Environment.OSVersion.ToString ();
		if (system_os.StartsWith ("Unix")) {
			this.OS = OperatingSystemType.Linux;
		...
		} else if (system_os.StartsWith ("Microsoft")) {
			...
			// Don't actually have any OSX data yet...
			this.OS = OperatingSystemType.OSX;
	...
	protected virtual void launch_game () {
		...
		if (File.Exists (binaryPicker.Filename)) {
			...
			using (Process p = Process.Start (binary.StartInfo)) {
				...
				p.WaitForExit ();
				txtOutput.Buffer.Clear ();
				txtOutput.Buffer.Text += errors;
				txtOutput.Buffer.Text += output;
			}
		}
	}
